...
export default function BillingsTablePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const billingData = await strapiApi.getInteriorDesignBillings().catch(() => ({ data: [] }));
  ...
  const loadSales = async () => {
    try {
      const salesData = await strapiApi.getInteriorDesignSales().catch(() => ({ data: [] }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingEntry && entryId) {
        await strapiApi.updateInteriorDesignBillings(entryId, submitData);
      } else {
        await strapiApi.createInteriorDesignBillings(submitData);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await strapiApi.deleteInteriorDesignBillings(entryId);
  ...
  return (
    <Box>
      ...
      {/* Sub Navigation Tabs */}
      <BranchCashflowTabs branchPath="/interior-design" branchName="Interior Design" />
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredBillings.length > 0 ? (
                        ...
                        <TableRow>
                          <TableCell colSpan={12} align="center" sx={{ py: 4 }}><Typography variant="body2" color="text.secondary">No billings match the selected filters</Typography></TableCell>
                        </TableRow>
                      )}
                    </TableBody>
                  </Table>
