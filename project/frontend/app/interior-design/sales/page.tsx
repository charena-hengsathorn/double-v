  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingEntry && entryId) {
        await strapiApi.updateInteriorDesignSales(entryId, submitData);
      } else {
        await strapiApi.createInteriorDesignSales(submitData);
      }
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedEntries).map(async (entryId) => {
        try {
          await strapiApi.deleteInteriorDesignSales(entryId);
        } catch (err: any) {
