  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingEntry && entryId) {
        await strapiApi.updateConstructionSales(entryId, submitData);
      } else {
        await strapiApi.createConstructionSales(submitData);
      }
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedEntries).map(async (entryId) => {
        try {
          await strapiApi.deleteConstructionSales(entryId);
        } catch (err: any) {
