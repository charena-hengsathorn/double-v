...
export default function ConstructionBillingsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const billingData = await strapiApi.getConstructionBillings().catch(() => ({ data: [] }));
  ...
  const loadSales = async () => {
    try {
      const salesData = await strapiApi.getConstructionSales().catch(() => ({ data: [] }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingEntry && entryId) {
        await strapiApi.updateConstructionBillings(entryId, submitData);
      } else {
        await strapiApi.createConstructionBillings(submitData);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await strapiApi.deleteConstructionBillings(entryId);
  ...
  return (
    <Box>
      ...
      {/* Sub Navigation Tabs */}
      <BranchCashflowTabs branchPath="/construction" branchName="Construction" />
          ...
          <Box sx={{ flex: 1, overflow: 'auto' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <TextField
                ...
                helperText="Calculated as Amount - Construction Cost"
                inputProps={{ min: 0, step: 0.01 }}
                sx={{
